...
| Platform | File |
|---|---|
| macOS 12+ (Apple silicon and Intel) | `Brainlogs_x.y.z_universal.dmg` |
| Windows 10+ | `Brainlogs_x.y.z_x64-setup.msi` |
...
- **macOS** asks for *Accessibility* so Brainlogs can read window titles and on-screen text through the accessibility tree. No screen recording permission is requested: Brainlogs never takes screenshots.
...
```bash
git clone https://github.com/ShivankK26/Brainlog && cd Brainlogs
