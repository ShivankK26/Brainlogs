...
export type PlaceHistory = { place: Place; visits: Array<Visit & { change: Change | null }>; facts: PlaceFact[]; owed: Owed[] };
export type Owed = { id: string; text: string; direction: "you" | "them"; who: string; status: string; dueAt: string | null };
export type Recall = { place: Place; visits: number; firstSeen: string | null; lastSeen: string | null; totalMs: number; previousVisit: Visit | null; change: Change | null; facts: PlaceFact[]; people: string[]; owed: Owed[]; withPeople: string[]; eventIds: string[] };
