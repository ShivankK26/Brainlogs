...
const PROMISE = /\b(?:i(?:'| wi)ll|i will|i'm going to|i am going to|i'll get you|will do|on it|let me|i'll send|i'll share|i'll have|going to)\b/i;
 ...
 */
export const PREVIEW = /^(?:your\s+)?message,|\breceived from\b|\bsent (?:from|to)\b|\bdelivered\b|\bview (?:message|profile)\b|\bnew message from\b|\b\d{1,2}:\d{2}\s*(?:am|pm)\b.*,|\bunread\b|\bnotification|\d{1,2}(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*at\d/i;
/** Invisible direction/zero-width marks that chat apps prepend; they break `^` anchors. */
export const INVISIBLE = /[\u200b-\u200f\u202a-\u202e\u2060-\u2064\ufeff]/g;
/** Acronyms and short all-caps tokens ("CGPA", "CTC") are fields, not people. */
export const ACRONYM = /^[A-Z][A-Z0-9.&-]{1,6}$/;
/** Mass appeals and forwards ask everyone, so they ask nobody in particular. */
export const BROADCAST = /\b(?:share this (?:message|post)|forward(?:ed)? (?:this|as received|message)|anyone you know|friends,? family|spread the word|urgent(?:ly)? (?:need|required)|blood (?:donor|donation|group|needed)|donat(?:e|ion|ors?)\b|please (?:circulate|forward|share)|pass (?:this|it) on|to all (?:members|groups)|dear all|hi all|hello everyone|immediately if|call\/message|seeking (?:a |an )?(?:role|roles|position|opportunit))/i;
...
function clean(s: string): string {
  return s.replace(INVISIBLE, "").replace(/\s+/g, " ").replace(/^[\s"'“”]+|[\s"'“”]+$/g, "").trim();
}
...
export function extractCommitments(e: Event, opts: { contact: string | null; now?: Date }): CommitmentCandidate[] {
  ...
  const turns = speakerTurns(e.text.replace(INVISIBLE, ""));
  ...
  for (const turn of turns) {
    for (const sentence of turn.text.split(/(?<=[.!?])\s+|\n/)) {
      ...
      // A speaker that is a form label, an acronym or a group name ("Batch", "CGPA") is not a counterpart.
      if (!turn.self && (LABEL_WORDS.has(turn.name.toLowerCase()) || ACRONYM.test(turn.name) || (!/\s/.test(turn.name) && turn.name.length < 3))) continue;
      ...
      if (other === "them" || LABEL_WORDS.has(other.toLowerCase()) || ACRONYM.test(other)) continue;
