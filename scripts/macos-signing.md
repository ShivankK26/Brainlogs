...
Open `apps/desktop/src-tauri/target/release/bundle/macos/Brainlog.app` (or the `.dmg` beside it). Drag the `.app` to `/Applications`. Day-to-day: open **Brainlog** — no npm terminals.
...
```bash
npm run tauri icon path/to/icon-1024.png -w @brainlog/desktop
```
...
**System Settings → Privacy & Security → Accessibility → Brainlog**
