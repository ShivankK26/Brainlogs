...
- macOS builds were **ad-hoc signed** (`signingIdentity: "-"`) through 1.0.10; from 1.0.11 releases carry a self-signed identity so the Accessibility grant survives updates (ADR 0012). Either way Gatekeeper shows "Open Anyway" instead of "damaged". Distribution is direct download only; `install.sh` fetches with `curl`, which sets no quarantine attribute, so users who prefer the terminal see no dialog at all.
...
- **OS keychain is opt-in (`BRAINLOG_USE_KEYCHAIN=1`), the default is the 0600 `master.key` file.** In practice a locked or password-mismatched login keychain makes every `security` read prompt for the keychain password, which the first user hit on every core start. The key protects `secrets.enc.json`, which sits in the same user-only directory as the database, so the file offers the same boundary. Writes to the keychain now recreate the item (never `-U`) and are verified by a read-back before the file copy is blanked.
